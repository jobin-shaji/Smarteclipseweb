<li class="treeview">
    <a href="#">
      <i class="fa fa-tablet"></i>
      <span>GPS</span>
      <span class="pull-right-container">
      <i class="fa fa-angle-left pull-right"></i>
      </span>
  </a>
  <ul class="treeview-menu" style="display: none;">
      <li><a href=""><i class="fa fa-plus"></i>Add GPS</a></li>
      <li><a href="<?php echo e(url('/users')); ?>"><i class="fa fa-list"></i> List GPS</a></li>
  </ul>
</li>

<li class="treeview">
    <a href="#">
      <i class="fa fa-bus"></i>
      <span>Vehicle</span>
      <span class="pull-right-container">
      <i class="fa fa-angle-left pull-right"></i>
      </span>
  </a>
  <ul class="treeview-menu" style="display: none;">
      <li><a href=""><i class="fa fa-plus"></i>Add Vehicle</a></li>
      <li><a href="<?php echo e(url('/users')); ?>"><i class="fa fa-list"></i> List Vehicles</a></li>
  </ul>
</li>

<li class="treeview">
    <a href="#">
      <i class="fa fa-user-plus"></i>
      <span>Dealer</span>
      <span class="pull-right-container">
      <i class="fa fa-angle-left pull-right"></i>
      </span>
  </a>
  <ul class="treeview-menu" style="display: none;">
      <li><a href=""><i class="fa fa-plus"></i>Add Dealer</a></li>
      <li><a href="<?php echo e(url('/users')); ?>"><i class="fa fa-list"></i> List Dealers</a></li>
  </ul>
</li>

<li class="treeview">
    <a href="#">
      <i class="fa fa-user-plus"></i>
      <span>Sub Dealer</span>
      <span class="pull-right-container">
      <i class="fa fa-angle-left pull-right"></i>
      </span>
  </a>
  <ul class="treeview-menu" style="display: none;">
      <li><a href=""><i class="fa fa-plus"></i>Add Sub Dealer</a></li>
      <li><a href="<?php echo e(url('/users')); ?>"><i class="fa fa-list"></i> List Sub Dealers</a></li>
  </ul>
</li>

<li class="treeview">
    <a href="#">
      <i class="fa fa-user-plus"></i>
      <span>User</span>
      <span class="pull-right-container">
      <i class="fa fa-angle-left pull-right"></i>
      </span>
  </a>
  <ul class="treeview-menu" style="display: none;">
      <li><a href=""><i class="fa fa-plus"></i>Add User</a></li>
      <li><a href="<?php echo e(url('/users')); ?>"><i class="fa fa-list"></i> List Users</a></li>
  </ul>
</li>


