<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Tax Invoice</title>
<style>
body {
  font-family: 'Helvetica Neue', sans-serif;
  background: #fff;
  margin: 0;
  padding: 0;
}
.invoice-container {
  max-width: 900px;
  margin: 40px auto;
  background: #fdfdfd;
  /* border: 1px solid #ccc; */
  box-shadow: 0 0 8px rgba(0,0,0,0.01);
  padding: 40px;
  border-radius: 4px;
}
.header {
  text-align: center;
  margin-bottom: 30px;
  border-bottom: 1px solid #8ec430;;
}
.header h1 {
  font-size: 32px;
  color: #444;
  margin: 0;
}
.header .subtext {
  font-size: 14px;
  color: #888;
}
.section-title {
  font-size: 18px;
  margin: 20px 0 10px;
  color: #333;
  border-bottom: 2px solid #eee;
  padding-bottom: 4px;
}
.info-table, .items-table, .summary-table {
  width: 100%;
  border-collapse: collapse;
  margin-bottom: 20px;
}
.info-table td, .items-table th, .items-table td, .summary-table td {
  border: 1px solid #ddd;
  padding: 8px;
  vertical-align: top;
}
.items-table th {
  background-color: #8ec430;
  color: #fff;
  font-size: 14px;
}
.summary-table td {
  font-size: 14px;
}
.summary-table td:last-child, .items-table td:last-child {
  text-align: right;
}
.footer {
  text-align: center;
  color: #888;
  font-size: 12px;
  margin-top: 30px;
  padding-top: 10px;
  border-top: 1px solid #eee;
}
</style>
</head>
<body>

<div class="invoice-container">
  <div class="header">
  <img src="data:image/png;base64,{{ base64_encode(file_get_contents('images/logo.png')) }}" class="image img-thumbnail" height="50px" width="50px" /></br>
    <h1>Tax Invoice</h1>
    <div class="subtext">ORIGINAL FOR RECIPIENT</div>
  </div>

  <div class="section-title">Invoice Parties</div>
  <table class="info-table">
    <tr>
      <td><br>
          VST MOBILITY SOLUTIONS PVT.LTD<br>
          1x/92 C PUTHIYA ROAD NEAR NSS KARAYOGAM HALL<br>
          EROOR PO THRIPINITHURA<br>
          Phone: 7994498567<br>
          Email: vstadmin@vstmobility.com<br>
          GSTIN: 32AAFCV2429L1Z9 | State: 32-Kerala</td>
      <td><strong>To:</strong><br>

      THE CHAIRMAN & MANAGING DIRECTOR<br>
      KERALA STATE ROAD TRANSPORT CORPORATION<br>
      TRANSPORT BHAVAN ,EAST FORT<br>
      THIRUVANANTHAPURAM-695023<br>
      GSTIN : 32AABCK1315J2Z0<br>
          Invoice No.:{{$gps->order_id}}<br>
          Date:  {{$gps->gps->pay_date}} <br>
          Sales Person: namitha</td>
    </tr>
  </table>

  <div class="section-title">Items</div>

@php

$totalAmount = $gps->total_amount; // Example total amount inclusive of 18% GST
  $gstRate = 18; // Combined GST
  $taxMultiplier = 1 + ($gstRate / 100);

  $baseAmount = $totalAmount / $taxMultiplier;
  $totalGST = $totalAmount - $baseAmount;
  $cgst = $sgst = $totalGST / 2;
  
  @endphp
<table class="items-table">
  <tr>
    <th>#</th>
    <th>Item Name</th>
    <th>Qty</th>
    <th>Unit Price</th>
    <th>GST</th>
    <th>Total</th>
  </tr>
  <tr>
    <td>1</td>
    <td>
    CMC Charge(March2024 august2024)
    <p><strong>IMEI & VEHICLE NO:</strong>{{$gps->gps->imei}} </br>{{$gps->gps->vehicle_no}}</p>
  </td>
  
    <td>1 Nos</td>
    <td> {{number_format($baseAmount,2)}}</td>
    <td> {{number_format($totalGST,2)}} (18%)</td>
    <td> {{$gps->total_amount}}</td>
  </tr>
</table>
<table class="summary-table">
  <tr><td>Sub Total</td><td>{{number_format($baseAmount,2)}}</td></tr>
  <tr><td>SGST@9%</td><td> {{number_format($sgst,2)}}</td></tr>
  <tr><td>CGST@9%</td><td>{{number_format($cgst,2)}}</td></tr>
  <tr style="background-color: #8ec430;"><td><strong>Total</strong></td><td><strong> {{$gps->total_amount}}</strong></td></tr>
  <tr><td>Received</td><td>{{$gps->total_amount}}</td></tr>
  <tr><td>Balance</td><td> 0.00</td></tr>
</table>






<p><strong>Invoice Amount in Words:</strong>{{$words}}</p>
<p><strong>Payment Mode:</strong> SBI VST IOT</p>
<p><strong>Pay To:</strong> STATE BANK OF INDIA, PALARIVATTOM, ERNAKULAM<br>
Account No.: 42037350382 | IFSC: SBIN0004312</p>

  <div class="footer">
  <img src="data:image/png;base64,{{ base64_encode(file_get_contents('images/sign.jpg')) }}" class="image img-thumbnail" height="50px" width="50px" />

    Authorized Signatory<br>
    Thank you for your business!
  </div>
</div>

</body>
</html>
