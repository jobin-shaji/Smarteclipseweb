...
function GetVehicles(){
  var url = 'dash-vehicle-user-track';
  var purl = getUrl() + '/'+url ;
    ...
    var data = {};
    jQuery.extend(defaults, {alert:false});//options);
...
function UpdateVehicles(res)
{
  //console.log("res.length="+res.length);

  for (let i = 0; i < res.length; i++) {

    let lat = parseFloat(res[i].lat);
    let lon = parseFloat(res[i].lon);

    if(lat<1000 && lon<1000){

      if (!isNaN(lat) && !isNaN(lon)) {
          var lapsed = getSecondsElapsed(res[i].updated_at);


          if(isMarker(markers[res[i].imei])){

            markers[res[i].imei].setPosition({ lat: lat, lng: lon });
          }else{
            markers[res[i].imei] = new google.maps.Marker({
              position: { lat: lat, lng: lon },
              map: map,
              title: res[i].register_number,
              icon: {
                url: GetIcon(lapsed)
              }
            });
            markers[res[i].imei].addListener('click', function() {
              OpenEugInfo(markers[res[i].imei],res[i].imei);
          });
          }
      }
    }
...
function OpenEugInfo(mark,ime){
  if(eugInfoWindows==null)eugInfoWindows = new google.maps.InfoWindow();
  eugInfoWindows.setContent('<p>Please wait...</p>');
  eugInfoWindows.open(map, mark);

  var url = 'GetEugInfoView';
  var purl = getUrl() + '/'+url ;
    ...
    $.ajax({
        ...
        success: function (res) {
            eugInfoWindows.setContent(res);
        },
        error: function (err) {
          console.log(err);
            var message = (err.responseJSON)?err.responseJSON.message:err.responseText;
            eugInfoWindows.setContent('<h4 style="color:red;">Server error</h4>');
        }
    });

}
function GetIcon(lapse){
  if(lapse<300)return 'https://maps.google.com/mapfiles/ms/icons/blue.png';
  else if(lapse<1800)return 'https://maps.google.com/mapfiles/ms/icons/orange.png';
  else return 'https://maps.google.com/mapfiles/ms/icons/red.png';
}
function OnVehicleClicked(ime){
  if(!isMarker(markers[ime]) || markers[ime].getPosition().lat()>1000){
    alert("Invalid GPS data");
  }else{
    map.panTo(markers[ime].getPosition());
    OpenEugInfo(markers[ime],ime)
  }
}
function isMarker(obj) {
  return obj instanceof google.maps.Marker;
}
function getSecondsElapsed(givenTime) {
  var givenDate = new Date(givenTime.replace(' ', 'T'));
...
//___________________________________________________________________________EUGENE
// check each 10 sec
/*eugene
window.setInterval(function() {
 if (track_flag == 0) {
