...
class GpsRecordController extends Controller {
    ...
    public function gpsUnprocessedDateWiseRecord()
    {
        ...
        if( $this->imei != '' && $this->date != '' )
        {
            $data   = (new VltData())->getUnprocessedData($this->imei, $this->date);
        }

        return view('GpsConfig::gps-unprocessed-daily-record', [ 'imei_serial_no_list' => $imei_serial_no_list, 'data' => $data, 'filters' => $filters ]);
    }
    ...
    public function exportUnprocessedData(Request $request)
    {
        ob_end_clean();
        ob_start();
        return Excel::download(new GpsUnprocessedDataReportExport($request->imei,$request->date), 'gps-unprocessed-data-report.xlsx');
    }
