...
<section class="hilite-content">
     ...
     <form  method="POST" action="{{route('etm.create.p')}}">
        {{csrf_field()}}
      <div class="row">
          <div class="col-md-6">
              ...
              <div class="form-group has-feedback">
                ...
                <input type="date" class="form-control {{ $errors->has('purchase_date') ? ' has-error' : '' }}" placeholder="Purchase Date" name="purchase_date" value="{{ old('purchase_date') }}" required> 
              ...
              @endif
               ...
               <div class="form-group has-feedback">
                <label class="srequired">Depot</label>
                   ...
                   <select class="form-control {{ $errors->has('to_depot') ? ' has-error' : '' }}" placeholder="Depot" name="depot"required>
                  <option selected disabled>Select</option>
                  @foreach($depots as $depot)
                  <option value="{{$depot->id}}">{{$depot->name}}||{{$depot->code}}</option>  
                  @endforeach
                 </select>
              ...
              @if ($errors->has('depot'))
                <span class="help-block">
                    <strong class="error-text">{{ $errors->first('depot') }}</strong>
                </span>
              @endif
           </div>
