...
$key        = (isset($_GET['new_service_search_key'])) ? $_GET['new_service_search_key'] : '';
?>
  <div class="page-wrapper_new">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
          <li class="breadcrumb-item active" aria-current="page"><a href="/home">Home</a>/New Service Jobs List</li>
          <b> New  Service Jobs List</b>
      </ol>
    ...
    <div align="right" class="search-1">
                 ...
                 <div class="row">
                    <div class="col-lg-8">
                      <div class="form-group" style="width: 100%;">
                         <input type="text" class="form-control" placeholder="Enter Job Code, Serial No,user name,End User Details" name="new_service_search_key" id="new_service_search_key" value="{{ $key }}">
                      </div>
                    </div>
          ...
          <thead class="indigo white-text">
              ...
              @foreach($servicer_jobs  as $key => $servicer_job)
                ...
                <td style="width:4%;">{{ (($perPage * ($page - 1)) + $loop->iteration) }}</td>
                    ...
                    <td> <a href="/servicejob/{{Crypt::encrypt($servicer_job->id)}}/servicedetails"class='btn btn-xs btn-info'><i class='glyphicon glyphicon-map-marker'></i>Job Completion</a></td>      
                  
                    @else
                    <td><a href="/servicejob/{{Crypt::encrypt($servicer_job->id)}}/servicedetails"class='btn btn-xs btn-info'><i class='glyphicon glyphicon-map-marker'></i>View</a</td>     
                    @endif
                </tr>
                @endforeach
            </tbody>
           ...
           .table td, .table th 
      {
        ...
        @section('script')
          ...
          {
            document.getElementById('new_service_search_key').value = '';
          }
