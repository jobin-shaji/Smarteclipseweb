...
where pay_status=1  group by employee_code order by employee_code desc) limit 1;");
      ...
      where pay_status=1  AND  pay_date='" . Date('Y-m-d') . "' group by employee_code order by employee_code desc) limit 1;");
        ...
        // Compute homepage best performers matching gpsrenewaldailyreport logic
        $startDate = Carbon::today()->startOfDay();
        $endDate = Carbon::today()->endOfDay();

        $topDayQuery = DB::table('gps_orders as o')
        ...
        $topByCountToday = (clone $topDayQuery)
            ->select('u.username', DB::raw('COUNT(o.id) as renewal_count'))
            ->groupBy('o.sales_by', 'u.username')
            ->orderByDesc('renewal_count')
            ->first();

        $hp_best_performer_day = $topByCountToday ? $topByCountToday->username : "";

        $topAllQuery = DB::table('gps_orders as o')
            ->join('gps_summery as g', 'g.id', '=', 'o.gps_id')
            ->leftJoin('users as u', 'u.id', '=', 'o.sales_by')
            ->whereNotNull('o.sales_by');

        $topByCountAll = (clone $topAllQuery)
            ->select('u.username', DB::raw('COUNT(o.id) as renewal_count'))
            ->groupBy('o.sales_by', 'u.username')
            ->orderByDesc('renewal_count')
            ->first();

        $hp_best_performer = $topByCountAll ? $topByCountAll->username : "";
        ...
        return response()->json([
            ...
            'status'                    =>  'dbcount',
            "best_performer"            => $hp_best_performer,
            "best_performer_day"        => $hp_best_performer_day,
            "total_renewal"             => $total_renewal,
            "todays_renewal"            => $todays_renewal,
...
where id_user=?", [$userid]);
        ...
        if ($mode === "check") {
            ...
            if ($regno !== 15) {
                ...
                if ($exists === 1) {
                    ...
                    $exists = 0;
                    $result = DB::select(
                        "select m_id from tbl_user_vs_vehicles where REPLACE(REPLACE(register_number_vehicles , ' ', ''), '-', '')=? and id_user=?",
            ...
            $inserted = DB::insert(
                "insert into tbl_user_vs_vehicles (register_number_vehicles,id_user) values (?,?)",
                [$regnoRetrived, $userid]
            );
            if ($inserted) {
            ...
            $deleted = DB::delete(
                "delete from tbl_user_vs_vehicles where REPLACE(REPLACE(register_number_vehicles , ' ', ''), '-', '')=?",
                [$regno]
            );
            if ($deleted) {
    ...
    public function UserActivities(Request $request)
    {
        ...
        if ($mode === 'search' || $mode === 'activate' || $mode === 'password') {
            ...
            $result = DB::select(
                "select id,mobile,email,username,role from users where id=? or username=? or mobile=? or email=?",
                [$user, $user, $user, $user]
            );
            ...
            if ($mode === 'activate') {
                ...
                if (!empty($hisVehicles)) {
                    foreach ($hisVehicles as $vehicle) {
                        $vehicleCount++;
                        $hisVehicle = DB::select(
                            "select * from tbl_user_vs_vehicles where id_user=? and register_number_vehicles=?",
                            [$result[0]->id, $vehicle->register_number]
                        );
                        if (empty($hisVehicle)) {
                            $res = DB::insert(
                                "insert into tbl_user_vs_vehicles (id_user,register_number_vehicles) values (?,?)",
                                [$result[0]->id, $vehicle->register_number]
                            );
                            if ($res) {
                                $addedCount++;
