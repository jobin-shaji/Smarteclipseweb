...
////////////////////////////COMPLAINT TYPE/////////////////////////////////////////////
    ...
    {
        if(\Auth::user()->hasRole('client|sub_dealer|root|trader|Call_Center')){
    ...
    public function getComplaints()
    {
            ...
            else if(\Auth::user()->hasRole('sub_dealer'))
            {
                ...
                $created_by      =   \Auth::user()->callcenter->id;
                echo $created_by;die;
                $complaints = $complaints->whereIn('created_by',$created_by);  
            }
            $complaints = $complaints->get();
           //return  print_r($complaints);die;
            ...
            ->addColumn('assigned_to', function ($complaints) { 
                ...
                { 
                    ...
                    {
                        return ($complaints->servicer)?$complaints->servicer->name:"";
                    }                    
                }
    ...
    public function create()
    {
        ...
        // $devices=GpsStock::with('gps')->where('client_id',$client_id)->get();
          ...
          $devices = Vehicle::whereIn('gps_id', function($query) use($call_id)  {
                $query->select('gps_id')
    ...
    public function save(Request $request)
    {
        ...
        if(\Auth::user()->hasRole(['Call_Center'])){
            $created_by= \Auth::user()->callcenter->id;
             $vehil=Vehicle::where('gps_id',$request->gps_id)->first();
             $client_id=$vehil->client_id;
    ...
    public function assignComplaintToServicer(Request $request)
    {
        ...
        $role       =   User::select('role_id')->where('id', $client->user_id)->first();
        if($role == null || $role == '')
        {
            $role = 1;
        }
