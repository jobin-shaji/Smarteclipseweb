                ...
                <div class="col-md-6">
                  <div class="form-group has-feedback form-group-1 mrg-rt-5">
                    ...
                    <select class="form-control select2" name="gps_id" data-live-search="true" title="Select GPS" required>
                      ...
                      @if($vehicle->gps)
                      <option value="{{$vehicle->gps_id}}" @if($vehicle->is_returned == 1){{"disabled"}} @endif>{{$vehicle->register_number}}-IMEI:{{$vehicle->gps->imei}}@if($vehicle->is_returned == 1){{" -- Returned"}} @endif</option>
