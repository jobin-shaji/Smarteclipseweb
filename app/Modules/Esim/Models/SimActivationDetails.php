...
class SimActivationDetails extends Model
{
	...
	{
		$query = "SELECT
        r.name as role, us.username,cl.name,gps.imei,
        ...
        LEFT JOIN model_has_roles as mhr ON mhr.model_id = us.id AND mhr.model_type = 'App\\\\Models\\\\User'
        LEFT JOIN roles as r ON r.id = mhr.role_id
		WHERE
        (date(sad.expire_on) BETWEEN '$from_date' AND  '$to_date')
		group by r.name, us.username, cl.name, gps.imei";
