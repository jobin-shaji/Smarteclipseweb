...
class SubscriptionController extends Controller 
{
    ...
    public function createSubscription()
    {
        $countries=Country::select([
            ...
            // dd($plans);
        return view('Subscription::subscription-add',['countries'=>$countries]);
    }
    // save subscription
    ...
    public function detailsSubscription(Request $request)
     {
        ...
        $subscription=Plan::with(['country'])->find($decrypted_id);
    ...
    {
        $decrypted_id = Crypt::decrypt($request->id);
        $subscription = Plan::find($decrypted_id);
        if($subscription == null)
        {
            return view('Subscription::404');
        }
        ...
        return view('Subscription::subscription-edit',['subscription' => $subscription,'countries' => $countries]);
    }
    ...
    {
        $subscription=Plan::withTrashed()->find($request->id);
        if($subscription == null){
           return response()->json([
                'status' => 0,
                'title' => 'Error',
