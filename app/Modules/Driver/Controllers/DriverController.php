...
class DriverController extends Controller {
    ...
    public function save(Request $request)
    {
        ...
        $eclipse_key="eclipse";
            ...
            User::where('username', $request->mobile)->first()->assignRole('Driver');
            ...
            $client = Driver::create([
                ...
                'mobile' => $request->mobile,
                'duty_schedule' => $request->duty_schedule,
                'salary_type' => $request->salary_type,
                'work_hrs' => $request->work_hrs??"",
                'salary' => $request->salary??$request->percentage,
    ...
    public function getDriverlist(Request $request)
    {
        ...
        ->addIndexColumn()
        ->editColumn('salary_type', function ($driver) {
            if($driver->salary_type=="Daily"){
                return  "Daily Wages";
            }
            if($driver->salary_type=="Commission"){
                return  "Commission Based";
            }
            if($driver->salary_type=="Trip" ){
                return  "Trip Based";
            }
        ...
        ->editColumn('licence_validity', function ($driver) {
             if($driver->licence_validity < Date('YYYY-MM-DD')){
                return  "Validity Expired";
            }else{
                 return ($driver->licence_validity)?$driver->licence_validity:'';
            }
    ...
    public function update(Request $request)
    {
        ...
        $driver->duty_schedule = $request->duty_schedule;
        $driver->salary_type = $request->salary_type??"";
        $driver->work_hrs = $request->work_hrs??"";
        if($request->salary_type=="Commission"){
            $driver->salary = $request->percentage??"";
        }else{
            $driver->salary = $request->salary??"";
        }
        $driver->licence_no = $request->licence_no;
        $driver->licence_validity = $request->licence_validity;
        $driver->break_time = $request->break_time;
        $driver->save();
