...
use App\Exports\ClientVehiclesExport;
...
class VehicleController extends Controller
{
    ...
    public function exportClientVehicles2wiithoutcertificate()
    {
        ob_end_clean();
        ob_start();
        ...
        $vehicles = Vehicle::where('client_id', $client_id)
            ->whereHas('gps', function ($q) {
                $q->where('pay_status', 1)
                  ->whereNotNull('warrenty_certificate');
            })
            ->with([
                'vehicleType:id,name',
                'driver:id,name',
                'gps:id,imei,serial_no,validity,pay_status,warrenty_certificate,installation_date'
            ])
            ->get();
    
        return Excel::download(
            new ClientVehiclesExport($vehicles),
            'client-vehicles-' . $client_id . '.xlsx'
        );
    }
     public function exportClientVehicles2_withcertificate()
    {
        ob_end_clean();
        ob_start();
        ...
        $vehicles = Vehicle::where('client_id', $client_id)
            ->whereDoesntHave('gps', function ($q) {
                $q->where('pay_status', 1)
                  ->whereNotNull('warrenty_certificate');
            })
            ->with([
    ...
    public function exportClientVehicles()
    {
        ob_end_clean();
        ob_start();
        ...
        return Excel::download(
            new ClientVehiclesExport($client_id),
            'client-vehicles-' . $client_id . '.xlsx'
        );
    }
    ...
    public function getVehicleList2()
    {
        if (\Auth::user()->hasRole('Driver')) {
    
            $id = \Auth::user()->driver->id;
            $vehicles = (new Vehicle())->getVehicleListBasedOnDriver($id);
        ...
        // ? FILTER: allow only vehicles that pass the condition
        $vehicles = $vehicles->filter(function ($vehicle) {
            return optional($vehicle->gps)->pay_status
                && optional($vehicle->gps)->warrenty_certificate;
        });
        ...
        return DataTables::of($vehicles)
            ...
            ->addColumn('driver', function ($vehicles) {
                return $vehicles->driver_id
                    ? optional($vehicles->driver)->name
                    : 'Not assigned';
            })
    
            ->addColumn('installation_date', function ($vehicles) {
                return $vehicles->installation_date;
            })
    
            ->addColumn('validity', function ($vehicles) {
                return $vehicles->validity_date ?? 'Not Renewed';
            })
    
            ->addColumn('renewed', function ($vehicles) {
                return optional($vehicles->gps)->pay_status
                    ? 'Renewed'
                    : 'Not Renewed';
            })
            ...
            ->addColumn('action', function ($vehicles) {
                ...
                if ($vehicles->is_returned == 1) {
                    return "<b style='color:red'>GPS Returned</b>";
                }
    
                $a = "
                    <a href='{$b_url}/vehicles/" . Crypt::encrypt($vehicles->id) . "/location'
                ...
                if (!\Auth::user()->hasRole('Driver')) {
                    $a .= "
                        <a href='{$b_url}/vehicles/" . Crypt::encrypt($vehicles->id) . "/assign'
                           class='btn btn-xs btn-info'>Assign Driver</a>
    
                        <a href='{$b_url}/vehicles/" . Crypt::encrypt($vehicles->id) . "/details'
                           class='btn btn-xs btn-info'>View/Edit</a>
                    ";
                }
    
                if (\Auth::user()->hasRole('client')) {
                    $a .= "
                        <a href='{$b_url}/uploads/{$vehicles->gps->warrenty_certificate}'
                           class='btn btn-xs btn-warning' download>
                           Download Certificate
                        </a>
    
                        <a href='{$b_url}/download-invoice/{$vehicles->gps->id}'
                           class='btn btn-xs btn-warning'>
                           Download Invoice
                        </a>
                    ";
    ...
    public function getVehicleList()
    {
        ...
        ->addColumn('driver', function ($vehicles) 
        {
            ...
            //return optional($vehicles->gps)->pay_status ?? 'No GPS';
        ...
        ->addColumn('installation_date', function ($vehicles) 
        {
            ...
            //$status=optional($vehicles->gps)->pay_status ??0;
          //  $installation_date=optional($vehicles->gps)->installation_date ??"";
            //return $validy;

          // $dates= $this->getRemainingMonthsAndNextDue($installation_date);
         
           return $vehicles->installation_date;
            //$vehicles->gps->installation_date
        ...
        ->addColumn('validity', function ($vehicles) 
        {
          ...
          // $dates= $this->getRemainingMonthsAndNextDue($installation_date);
           ...
           return $vehicles->validity_date;
            //$vehicles->gps->installation_date
            if($status)
            {
                $validy=optional($vehicles->gps)->validity ??"";
            ...
            else
            {
                ...
                return optional($vehicles->gps)->validity ?? 'Not Renewed';
        ...
        ->addColumn('action', function ($vehicles) 
        {
            ...
            if($vehicles->deleted_at == null)
            {
                if($vehicles->is_returned == 1)
                {
                   ...
                   ";
                   if(!\Auth::user()->hasRole('Driver')){
                    $a.="<a href=".$b_url."/vehicles/".Crypt::encrypt($vehicles->id)."/assign class='btn btn-xs btn-info' data-toggle='tooltip' title='View'>Assign a driver</i> </a>";
                    $a.="<a href=".$b_url."/vehicles/".Crypt::encrypt($vehicles->id)."/details class='btn btn-xs btn-info' data-toggle='tooltip' title='View'>View/Edit</i> </a>";
                    }
                    if(\Auth::user()->hasRole('client')){
                        $a.="<a href=".$b_url."/vehicles/".Crypt::encrypt($vehicles->id)."/sendSms class='btn btn-xs btn-warning' data-toggle='tooltip' title='View'>send sms </a>";
                         if(optional($vehicles->gps)->pay_status && optional($vehicles->gps)->warrenty_certificate){
                       

                            $a.="<a href=".$b_url."/uploads/".$vehicles->gps->warrenty_certificate." class='btn btn-xs btn-warning' data-toggle='tooltip' title='Download Certificate' download>Download Certificate </a>";
                            $a.="<a href=".$b_url."/download-invoice/".$vehicles->gps->id." class='btn btn-xs btn-warning' data-toggle='tooltip' title='Download Invoice'>Download Invoice </a>";
                  
                         }

                         if($vehicles->inv_generated){
                             $a.="<a href=".$b_url."/download-cmc/".$vehicles->gps_id." class='btn btn-xs btn-warning' data-toggle='tooltip' title='Download CMC'>Download CMC </a>";
                  
                         }

                        else if(optional($vehicles->gps)->pay_status){
                            ...
                            $a.= "<form  method='POST' action='/esim-post-renewal'>".csrf_field()."
                                <input type='hidden'  name='id' value='".$vehicles->gps_id."'>
    ...
    public function createVehicle(Request $request)
    {
        ...
        $imei=$request->imei??"";
        if($imei){
        $devices=Gps::whereNotIn('id', Vehicle::pluck('gps_id'))
    ->where('imei', 'like', '%'.$imei.'%')
    ->get();

        }else{
        //$devices=GpsTransfer::with('gpsTransferItems')->where('to_user_id',$client_user_id) ->get();
         $devices=(new GpsTransfer())->getuserGPS($client_user_id);
        }
    ...
    public function assignDriver(Request $request)
    {
        ...
        $drivers=Driver::select('id','name')
                ->where('client_id',$client_id)
                ->get();
        return view('Vehicle::vehicle-assign',['vehicle' => $vehicle,'drivers' => $drivers]);
    }
    ...
    public function getsendSms(Request $request)
    {
        ...
        $gps=Gps::select('imei','e_sim_number','id')
                ->where('id',$vehicle->gps_id)->first();
                
        return view('Vehicle::vehicle-sms',['vehicle' => $vehicle,'gps' => $gps]);
    }
    ...
    public function getbulksms()
    {
        ...
        $gps = Gps::select('imei','e_sim_number')
            ->join('vehicles','gps_summery.id','=','vehicles.gps_id')
            ->where('vehicles.client_id',$client_id)
			->orderBy('vehicles.id','DESC')->get();
		    return view('Vehicle::vehicle-bulksms',['gps' => $gps]);
    }


    public function postsendDsms(Request $request)
    {
        $gps=Gps::select('imei','e_sim_number' )
        ->where('id',$request->gps_id)->first();
        $sms=$request->sms; 
        $message ="SET%20".$sms ;
        $template_id = "1707174143336789808";
        $this->sendmessage($message, $gps->e_sim_number, $template_id);
    ...
    public function postsendBulksms(Request $request)
    {
        ...
        $esims=$request->esims;
        $sendsims= implode(', ',  $esims);
        $sms=$request->sms; 
        $message ="SET%20".$sms ;
        $template_id = "1707174143336789808";
        $this->sendmessage($message, $sendsims, $template_id);
        return redirect(route('vehicle')); 
        //return view('Vehicle::vehicle-assign',['vehicle' => $vehicle,'drivers' => $drivers]);
    }
    public function postassignDriver(Request $request)
    {
        $vehicles = Vehicle::where('driver_id', $request->driver_id)->first();
...
//dd(12345);
    ...
    public function locationTrack(Request $request)
    {
        ...
        $track_data=GpsHistory::select('latitude as latitude',
                      ...
                      'distance',
                      'fuel',
                      'battery_percentage as battery_percentage',
...
-------unsnapped location points-------------*/
    ...
    public function vehiclePlayback(Request $request)
    {
        ...
        if($is_table_exist_in_database)
        {
            ...
            $count_of_gpsdata               =   (new GpsHistory())->getCountGpsData($from_date,$to_date,$gps_in_vehicle,$gps_data_table);                  
            $total_index                    =   ceil($count_of_gpsdata / 30);
            $track_data                     =   (new GpsHistory())->getTrackData($from_date,$to_date,$gps_in_vehicle,$start_offset,$limit,$gps_data_table);
            $alerts_list                    =   [];
            if($track_data->count() > 0)
            {
                ...
                $alerts_list=(new Alert())->getAlertByBetweenDatetime($get_vehicle->gps_id,$from_date_time);
    ...
    public function getRemainingMonthsAndNextDue($previousDate)
    {
       // $previous = Carbon::createFromFormat('m/d/Y', $previousDate);
      
     // $old_date = explode('/', $previousDate); 
     //  $prev = $old_date[2].'-'.$old_date[0].'-'.$old_date[1];
        $prev=$previousDate;
        ...
        return [
            'adj_months'=>$adjustedDate->toDateString(),
            'prev'=>$uptDueDate->toDateString(),
            'prev1' => $previous,
