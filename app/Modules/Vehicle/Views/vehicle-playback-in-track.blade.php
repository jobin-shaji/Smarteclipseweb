               ...
               <div class="row row-mrg-1">
                  <span id="cover_date_time_picker">
                     <span id="cover_date_time_picker_cover_date">
                        <div class='col-sm-3'>
                           <div class="form-group">
                              ...
                              <div class="input-group date <?php if(\Auth::user()->hasRole('superior')){ echo 'datepickerSuperior'; }else if(\Auth::user()->hasRole('fundamental')){ echo 'datepickerFundamental'; } else if(\Auth::user()->hasRole('pro')){ echo 'datepickerPro'; }else if(\Auth::user()->hasRole('freebies')){ echo 'datepickerFreebies'; } else{ echo 'datepickerFreebies';}?>" id="<?php if(\Auth::user()->hasRole('superior')){ echo 'datepickerSuperior'; }else if(\Auth::user()->hasRole('fundamental')){ echo 'datepickerFundamental'; } else if(\Auth::user()->hasRole('pro')){ echo 'datepickerPro'; }else if(\Auth::user()->hasRole('freebies')){ echo 'datepickerFreebies'; } else{ echo 'datepickerFreebies';}?>">
                                 <input type='text' style="height: 33px;" class="form-control" id="playback_date" name="fromDate" required />
                        ...
                        <div class='col-sm-2'>
                           <div class="form-group">
                              ...
                              <div class="input-group date start_time">
                                 <input type="text" id="start_time_val" style="height: 33px;"class="form-control" name="toDate" required>
                                 <span class="input-group-addon">
                        ...
                        <div class='col-sm-2'>
                           <div class="form-group">
                              <label style="font-weight:bold">Stop Time</label>
                              <div class="input-group date stop_time">
                                 <input type="text" id="stop_time_val" style="height: 33px;"class="form-control" name="toDate" required>
                                 <span class="input-group-addon">
                                 <span class="glyphicon glyphicon-time"></span>
                                 </span>
                              </div>
...
--> <script src="{{asset('playback_assets/assets/js/plugin/jquery-ui-touch-punch/jquery.ui.touch-punch.min.js')}}"></script>
      ...
      <script async defer src="https://maps.googleapis.com/maps/api/js?key={{config('eclipse.keys.googleMap')}}"></script>

     
      <script type="text/javascript" src="https://maps.google.com/maps/api/js?key={{config('eclipse.keys.googleMap')}}&libraries=directions"></script>
 <script>
         var km_data                  = 0;
    ...
    //    var domIcon             = new google.maps.DomIcon(outerElement);
        var start_icon          = '{{asset("playback/assets/img/start.png")}}';
         var stop_icon           = '{{asset("playback/assets/img/flag.png")}}';
         var hidpi               = ('devicePixelRatio' in window && devicePixelRatio > 1);
         
         var alert_icon          = '{{asset("playback/assets/img/alert-icon.png")}}';
      ...
      var map = new google.maps.Map(document.getElementById('markers'), {
                center: { lat: 10.192656, lng: 76.386666 }, // Default center (San Francisco)
                zoom: 14
            });
            ...
            const request = {
                ...
                waypoints: [
                    ...
                    { location: { lat: 36.7783, lng: -119.4179 } }  // Fresno
                ],
    ...
    //    new google.mapsevents.Behavior(new H.mapevents.MapEvents(map)); // add behavior control
        // var ui = H.ui.UI.createDefault(map, maptypes); // add UI
         ...
         var playback_speed_changed   = false; 
         var vehicle_mode;
         ...
         function startPlayBack(){
                 ...
                 if($('#playback_date').val()== "")
                 {
                   alertify.alert('Date required').setHeader('<em> PLAYBACK</em>');
                   return false;
                 }
                ...
                if($('#playback_date').val() !="" && $('#start_time_val').val() != "" && $('#stop_time_val').val() != "" )
                 {
                  $('#cover_date_time_picker_cover_date').css('display','none');
                  $('#cover_date_time_picker').append('<div class="col-sm-6 col-date-outer"><span class="datetime_searched"> '+play_back_date+ ' ['+start_time+'-TO-'+stop_time+'] </span><span onclick="resetDate()" class="close-span-rt"><i class="fa fa-times"></i></span></div>');
                 }
         }
        ...
        function getLocationData()
        {
         ...
         function plotLocationOnMap()
         {
             ...
             if(location_data_que.length >0)
             {
                 ...
                 if(first_point==true){
                      ...
                      var madridMarker = marker.setPosition({lat:location_data_que[0].lat, lng:location_data_que[0].lng},{ icon: start_icon});
                 ...
                 if(last_offset==true && location_data_que.length ==0){
                      ...
                      var flag = marker.setPosition({lat:startPointLatitude, lng:startPointLongitude},{ icon: stop_icon});
                     // map.addObject(flag);
         ...
         function moveMarker(RotateDegree,lat,lng,vehicle_mode,date_time){
             ...
             //var domIcon =new google.maps.DomIcon(outerElement);
          /*   bearsMarkeronStartPoint =new google.maps.DomMarker({ lat:lat, lng:lng }, {
                 icon: domIcon
             });
         ...
         function addMarkerToGroup(group, coordinate, html) {
           var marker = new google.maps.Marker(coordinate,{icon: alert_icon});
           marker.setData(html);
           group.addObject(marker);
         }
         
         function addInfoBubble(lat,lng,alert,time) {
           var group = new google.maps.Group();
           map.addObject(group);
           group.addEventListener('tap', function (evt) {
             var bubble =  new google.maps.InfoWindow(evt.target.getGeometry(), {
