...
class ClientController extends Controller {
    ...
    public function getClientlist(Request $request)
    {
        ...
        ->addColumn('action', function ($client) {
             $b_url = \URL::to('/');
        if( isset($client->user) && $client->user->deleted_at == null && $client->deleted_at == null){
            return "
    ...
    public function getRootClient()
    {
        ...
        return DataTables::of($client)
        ->addIndexColumn()
        ->addColumn('user.mobile', function ($client) {
            return "";
        })
        ->addColumn('user.email', function ($client) {
            return "";
        })
...
if(!$client){
    ...
    $client  = Client::create([
        ...
        'address' => strtoupper($user->username),
        'latitude'=>NULL,
        'longitude'=>NULL,
        'country_id'=>101,
        'state_id'=>19,
        'city_id'=>2045,
        'latest_user_updates'=>date('Y-M-d')
        ]);
    $vehicles=Vehicle::where('client_id',$user->id)->first();
    $vehicles->client_id=$client->id;
    $vehicles->save();
}
 } */
