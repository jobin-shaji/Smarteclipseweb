...
use PDF;
use Carbon\Carbon;
use App\Exports\GpsRenewalExport;
...
class GpsReportController extends Controller 
{
    ...
    public function gpsrenewaldailyreport(Request $request)
    {
        /*
        |----------------------------------------------------------
        | Determine filter type (day / month / default today)
        |----------------------------------------------------------
        */
        $selectedDate  = $request->get('date');   // yyyy-mm-dd
        $selectedMonth = $request->get('month');  // yyyy-mm
    
        if ($selectedDate) {
            $startDate = Carbon::parse($selectedDate)->startOfDay();
            $endDate   = Carbon::parse($selectedDate)->endOfDay();
            $labelDate = Carbon::parse($selectedDate)->format('d M Y');
        }
        elseif ($selectedMonth) {
            $startDate = Carbon::parse($selectedMonth . '-01')->startOfMonth();
            $endDate   = Carbon::parse($selectedMonth . '-01')->endOfMonth();
            $labelDate = Carbon::parse($selectedMonth)->format('F Y');
        }
        else {
            $startDate = Carbon::today()->startOfDay();
            $endDate   = Carbon::today()->endOfDay();
            $labelDate = Carbon::today()->format('d M Y');
        }
        ...
        */
        ...
        /*
        |----------------------------------------------------------
        | 1. Top employee by renewal count
        |----------------------------------------------------------
        */
        $topByCount = (clone $baseQuery)
            ->select(
                'u.username',
                DB::raw('COUNT(o.id) as renewal_count')
            )
            ->groupBy('o.sales_by', 'u.username')
            ->orderByDesc('renewal_count')
            ->first();
    
        /*
        |----------------------------------------------------------
        | 2. Top employee by amount
        |----------------------------------------------------------
        */
        $topByAmount = (clone $baseQuery)
            ->select(
                'u.username',
        ...
        */
        $todaySales = (clone $baseQuery)
            ->select(
                DB::raw('COUNT(o.id) as total_renewals'),
    ...
    public function gpsRenewalExport(Request $request)
    {
        // VERY IMPORTANT
        if (ob_get_length()) {
            ob_end_clean();
        ...
        if ($date) {
            ...
            $fileName  = 'gps-renewals-' . $date . '.xlsx';
        } elseif ($month) {
            $startDate = Carbon::parse($month . '-01')->startOfMonth();
            $endDate   = Carbon::parse($month . '-01')->endOfMonth();
            $fileName  = 'gps-renewals-' . $month . '.xlsx';
        } else {
            $startDate = Carbon::create(2017, 1, 1)->startOfDay();
            $endDate   = Carbon::today()->endOfDay();
            $fileName  = 'gps-renewals-today.xlsx';
        }
    
        return \Maatwebsite\Excel\Facades\Excel::download(
            new \App\Exports\GpsRenewalExport($startDate, $endDate),
            $fileName
        );
    }
    ...
    public function gpsTransferReport(Request $request)
    {
        ...
        else if(\Auth::user()->hasRole('sales'))
        {
            ...
            {
                $transfer_details       =   $this->TransferSummaryOfDistributorForSales($from_date,$to_date,$download_type);
            }
            if($download_type == 'pdf')
            {
                $pdf                    =   PDF::loadView('GpsReport::gps-transfer-report-download',['transfer_summary' => $transfer_details['transfer_summary'], 'manufacturer_to_distributor_details'=> $transfer_details['manufacturer_to_distributor_details'], 'distributor_to_dealer_details' => $transfer_details['distributor_to_dealer_details'], 'dealer_to_sub_dealer_details'=> $transfer_details['dealer_to_sub_dealer_details'],'dealer_to_client_details' => $transfer_details['dealer_to_client_details'], 'sub_dealer_to_client_details' => $transfer_details['sub_dealer_to_client_details'], 'generated_by' => $transfer_details['generated_by'].' '.'( Sales Department )', 'generated_on' => date("d/m/Y h:i:s A"), 'from_date' => date('d/m/Y',strtotime($from_date)), 'to_date' => date('d/m/Y',strtotime($to_date))]);
                return $pdf->download('gps-transfer-report.pdf');
            }
            else
            {
     ...
     * 
     */
    public function TransferSummaryOfDistributorForSales($from_date,$to_date,$download_type)
    {
        $transfer_details           =   [];
        //from distributers
        ...
        {
            $sub_dealer_to_client_details           =   $this->detailedTransferReportBetweenTwoUsers($sub_dealer_user_ids,$client_of_sub_dealer_user_ids,$from_date,$to_date);
            $sales_details                          =   (new Salesman())->getSalesmanDetails(\Auth::user()->salesman->id);
            return [
                'transfer_summary'                      =>  $transfer_details,
                'manufacturer_to_distributor_details'   =>  [],
                'distributor_to_dealer_details'         =>  $distributor_to_dealer_details,
                ...
                'dealer_to_client_details'              =>  $dealer_to_client_details,
                'sub_dealer_to_client_details'          =>  $sub_dealer_to_client_details,
                'generated_by'                          =>  ucfirst(strtolower($sales_details->name)) 
            ];
        }
        else
        {
    ...
    public function gpsTransferReportTransactionDetails(Request $request)
    {
        ...
        {
            $logged_user            =   (new Salesman())->getSalesmanDetails(\Auth::user()->salesman->id)->name;
        }
    ...
    public function gpsRenewalPending(Request $request)
    {
        $clients=Client::all();
        $callcenter = Callcenter::all();
        return view('GpsReport::gps-renewal-pending',['clients' => $clients, 'callcenter' => $callcenter]);
        
    }

    public function renewallist(Request $request){
        $client  =  $request->client;
        $all_devices         =  (new Gps())->getAllPendingDevicesList();
        $all_devices ->get();  
         
         return DataTables::of($all_devices)


         ->editColumn('imei', function ($all_devices) {
            if($all_devices->imei){
                return $all_devices->imei;
              //  return str_repeat('*', strlen($all_devices->imei) - 4) . substr($all_devices->imei, -4);;
                }
       ...
       if(\Auth::user()->hasRole('sales')){
          if(!$all_devices->pay_status){
             $a.= "<form  method='POST' action='/esim-post-renewal' style='display:inline-block; margin-right:5px;'>".csrf_field()."
                 <input type='hidden'  name='id' value='".$all_devices->id."'>
                 <button type='submit' class='btn btn-primary btn-sm address_btn'>Renew</button>
             </form>"; 
          }else{
            $a.="Renewed";
          }
       }
       // Show assign button for all users (sales, root, etc) if device is not renewed
       if(!$all_devices->pay_status){
           $a.= "<button type='button' class='btn btn-success btn-sm assign-btn' data-gps-id='".$all_devices->id."'>Assign</button>";
       }
     return $a;  
...
->make();
    ...
    public function assignSingleDevice(Request $request)
    {
        try {
            \Log::info('=== assignSingleDevice START ===');
            \Log::info('Request data:', $request->all());
            \Log::info('User ID:', ['user_id' => \Auth::id()]);
            \Log::info('User:', ['user' => \Auth::user() ? \Auth::user()->username : 'not authenticated']);
            
            $request->validate([
                'gps_id' => 'required|integer',
                'callcenter_id' => 'required|integer',
            ]);
            
            \Log::info('Validation passed');

            // Use user_id for assigned_by (consistent with automation system)
            // Note: Column stores both user_id and salesman_id historically, but we'll use user_id going forward
            $assigned_by = \Auth::id();
            ...
            $assignment = SalesAssignGps::create([
                'callcenter_id' => $request->callcenter_id,
                'gps_id' => $request->gps_id,
                'assigned_by' => $assigned_by,
            ...
            return response()->json([
                'success' => true, 
                'message' => 'Device assigned successfully to call center',
                'debug' => [
            ...
            \Log::error('assignSingleDevice exception:', [
    ...
    public function deviceOnlineReport(Request $request)
    {
        ...
        if(\Auth::user()->operations){
            ...
            $logged_user_details            =   (new Operations())->getOperatorDetails(\Auth::user()->id);
