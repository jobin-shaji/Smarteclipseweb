...
class DeviceReturnController extends Controller 
{
    ...
    /**
     * Direct Device Return - returns device without client assignment or vehicle creation
     * Root only - for devices at dealer level needing immediate refurbishment
     */
    public function directReturnPage()
    {
        return view('DeviceReturn::direct-device-return');
    }

    /**
     * Process Direct Device Return
     */
    public function directReturnProcess(Request $request)
    {
        ...
        // Find GPS Stock
        $gps_stock = GpsStock::where('gps_id', $gps->id)->first();

        if (!$gps_stock) {
            return response()->json([
                'status' => 0,
                'message' => 'Device inventory record not found'
            ]);
        }

        // Check if already returned
        if ($gps_stock->is_returned == 1) {
            return response()->json([
                'status' => 0,
                'message' => 'Device is already marked as returned'
            ]);
        }
        ...
        try {
            ...
            // Update GPS table - mark as returned (NO IMEI suffix)
            $gps->is_returned = 1;
            $gps->refurbished_status = 1;
            $gps->save();

            // Update GPS Stock table - reset to root stock as refurbished
            // Keep is_returned = 0 so it shows in refurbished stock count
            $gps_stock->is_returned = 0;
            $gps_stock->refurbished_status = 1;
            $gps_stock->dealer_id = NULL;
            $gps_stock->subdealer_id = NULL;
            $gps_stock->client_id = NULL;
            $gps_stock->returned_on = date('Y-m-d H:i:s');
            $gps_stock->save();

            // Check if vehicle exists for this GPS - only update if it exists
            $vehicle = Vehicle::where('gps_id', $gps->id)
            ...
            if ($vehicle) {
                // Update vehicle
                $vehicle->is_returned = 1;
                $vehicle->is_reinstallation_job_created = 0;
                $vehicle->save();

                // Update vehicle GPS log
                $vehicle_gps_log = VehicleGps::where('vehicle_id', $vehicle->id)
                                             ->where('gps_id', $gps->id)
                                             ->first();
                if ($vehicle_gps_log) {
                    $vehicle_gps_log->gps_removed_on = date('Y-m-d H:i:s');
                ...
                // Delete geofences for this vehicle
                VehicleGeofence::where('vehicle_id', $vehicle->id)->delete();
            }

            DB::commit();

            $response_message = 'Device ' . $imei . ' returned successfully';
            if ($vehicle) {
                $response_message .= ' (Vehicle also marked as returned)';
            }
            ...
            return response()->json([
                ...
                'data' => [
                    ...
                    'vehicle_updated' => $vehicle ? true : false,
            ...
            return response()->json([
                'status' => 0,
                'message' => 'Error processing device return: ' . $e->getMessage()
            ]);
        }
    }
}
