...
class ServicerController extends Controller {
        ...
        return DataTables::of($servicer)
            ...
            ->addColumn('action', function ($servicer) {
                ...
                if($servicer->deleted_at == null){
                    ...
                    if($servicer->sales_id==""){
                        $a.=" <a href=".$b_url."/servicer/".Crypt::encrypt($servicer->id)."/assign_sales  class='btn btn-xs btn-primary'><i class='glyphicon glyphicon-eye-open'></i>Assign to sales</a>";
                    }
                    $a.="
    ...
    public function assignServicer()
    {
        ...
        $servicers      =   Servicer::select('id','name','type','status','user_id','deleted_by')
        ...
        $client         =   Client::select('id')
                            ->orderBy('id','desc')
                            ->first();
        return view('Servicer::assign-servicer',['servicers'=>$servicers,'client_id'=>$client->id]);
    }
    ...
    {
        $end_user_id    =   ( isset($request->id) ) ? Crypt::decrypt($request->id) : null;
...
//     public function clientGpsList(Request $request)   OLD  CODE
//     {
//         $user           =   $request->user();
//         $client_id      =   $request->client_id;
//         if($client_id != 0)
   ...
   public function updateChecklistRule()
     ...
     public function servicercompleteJobRules()
    {
        $rules = [
            ...
            'installation_photo' => 'required|mimes:jpg,jpeg,png|max:4096',
            'activation_photo' => 'required|mimes:jpg,jpeg,png|max:4096',
            'vehicle_photo' => 'required|mimes:jpg,jpeg,png|max:4096',
            // 'comment' => 'required',
            // 'driver'=>'required',
            'model'=>'required',
            'make'=>'required'
    ...
    public function assignSales(Request $request)
    {
        ...
        return view('Servicer::servicer-assign',['salesmans'=>$salesmans,'subdealer' => $subdealer,'user' => $user]);
    }
    ...
    public function updateSales(Request $request)
    {
        ...
        $subdealer->sales_id = $request->sales_id;
        $subdealer->save();
        $did = encrypt($user->id);
        $request->session()->flash('message', 'Servicer assign to sales successfully!');
        $request->session()->flash('alert-class', 'alert-success'); 
