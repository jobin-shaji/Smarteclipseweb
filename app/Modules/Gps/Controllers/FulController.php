...
class FulController extends Controller {
 ...
 public function FulListPage()
    {
        return view('Gps::ful-data-list');
    }
    public function getFulAllData(Request $request)
    {
      ...
      return response()->json([ 'data' => $this->processFulData($packet)]);
      }
    }

    public function processFulData($vlt_data){
      $this->vltDatafunction($vlt_data);
    ...
    public function vltDatafunction($dataArray){

      $url = "http://103.135.130.119:80";

      $post = array('vltdata'=>$dataArray);
