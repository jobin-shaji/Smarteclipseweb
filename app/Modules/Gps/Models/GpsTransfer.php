...
public function getuserGPS($userId){
    return DB::select("SELECT gp.id,gp.imei FROM gps gp inner join gps_transfer_items gi on gp.id=gi.gps_id inner join gps_transfers gt on gi.gps_transfer_id=gt.id where gt.to_user_id=".$userId);
