...
Route::group(['middleware' => ['web','auth','role:root|Driver|sales|client|Call_Center'] , 'namespace' => 'App\Modules\Gps\Controllers' ] , function() {
	...
	Route::any('/gps/updatePayment','GpsController@updatePayment')->name('gps.updatePayment.p');;
	...
	Route::get('/esim-renewal','GpsController@esimRenewal')->name('esim.esim-renewal');
	Route::post('/esim-post-renewal','GpsController@esimpostRenewal')->name('esim.esim-post-renewal');
	

//	Route::get('/gps/file/download/{id}','GpsController@downloadPF');
//fetch plans

		Route::post('/fetch-plan','GpsController@fetchPlan')->name('gps.fetch.plans');
	
//otp sms
		Route::post('/send-otp', 'GpsController@sendOtp')->name('send.otp');
		Route::post('/verify-otp', 'GpsController@verifyOtp')->name('verify.otp');
...
Route::group(['middleware' => ['web','auth','role:sub_dealer|dealer|root|trader|client|sales|Call_Center'] , 'namespace' => 'App\Modules\Gps\Controllers' ] , function() {
...
Route::group(['middleware' => ['web','auth','role:operations'] ,'namespace' => 'App\Modules\Gps\Controllers' ] , function() {
	...
	*/
	...
	Route::any('/stock-report-list','GpsController@stockReportList')->name('stock-report-list');
...
//for packet
Route::group(['middleware' => ['web','auth','role:Finance'] ,'namespace' => 'App\Modules\Gps\Controllers' ] , function() {
	
	Route::get('/simrenewal','GpsController@renewalgpsList')->name('simrenewal');
	Route::post('/gps-renewal-list','GpsController@getRenewalgpsList')->name('gps.renewal.list');
	Route::get('/sim/details/{id}/','GpsController@simDetails')->name('sim.details');
