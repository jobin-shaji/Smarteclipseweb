...
Route::group(['middleware' => ['web','auth','role:client'] , 'namespace' => 'App\Modules\Alert\Controllers' ] , function() {
	...
	Route::get('/all-alerts','AlertController@allGpsAlerts')->name('all-alerts');
	Route::post('/notification-alerts-list','AlertController@alertNotificationList')->name('notification.alerts.list');
...
Route::group(['namespace' => 'App\Modules\Alert\Controllers' ] , function() {
		Route::get('/packet','AlertController@packet')->name('packet');
	Route::post('/packet/create','AlertController@save')->name('packet.create.p');
	Route::post('/alert-count','AlertController@alertCount')->name('alert-count');
	Route::post('/alert-mark-as-read','AlertController@changeAlertStatus')->name('alert-mark-as-read');
	Route::post('/last-five-unread-alerts','AlertController@alertLstUnreadCount')->name('last-five-unread-alerts');
	Route::post('/last-seven-days-alerts','AlertController@alertLstSevenDays')->name('last-seven-days-alerts');
