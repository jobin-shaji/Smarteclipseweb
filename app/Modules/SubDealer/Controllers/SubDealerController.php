...
class SubDealerController extends Controller {
    ...
    {
        $user=User::get()->pluck('id')->toArray();
        ...
        ->with('sales:id,name')
        ->with('user:id,email,mobile,deleted_at')->whereIn('user_id',$user)
        ...
        ->addIndexColumn()
        ->editColumn('sales.name', function ($subdealers) {
            return $subdealers->sales ? $subdealers->sales->name : 'N/A';
        })      
        ->addColumn('working_status', function ($subdealers) {
            $b_url = \URL::to('/');
            $a="";
            if($subdealers->user->deleted_at == null && $subdealers->deleted_at == null){ 
                if($subdealers->sales_id==""){
                    $a.=" <a href=".$b_url."/sub-dealers/".Crypt::encrypt($subdealers->user_id)."/assign_sales  class='btn btn-xs btn-primary'><i class='glyphicon glyphicon-eye-open'></i>Assign to sales</a>";
                }
                $a.= "
    ...
    public function getSubDealerlist(Request $request)
    {
        ...
        ->addColumn('action', function ($subdealers) {
            $b_url = \URL::to('/');
            if($subdealers->user){
                if($subdealers->user->deleted_at =="" && $subdealers->deleted_at == ""){ 
                    $a="";
                    if($subdealers->sales_id==""){
                        $a.=" <a href=".$b_url."/sub-dealers/".Crypt::encrypt($subdealers->user_id)."/assign_sales  class='btn btn-xs btn-primary'><i class='glyphicon glyphicon-eye-open'></i>Assign to sales</a>";
                    }
    ...
    public function updateSales(Request $request)
    {
        ...
        $subdealer->save();
        $did = encrypt($user->id);
        $request->session()->flash('message', 'Dealer assign to sales successfully!');
        $request->session()->flash('alert-class', 'alert-success'); 
    ...
    public function assignSales(Request $request)
    {
        ...
        return view('SubDealer::sub-dealer-assign',['salesmans'=>$salesmans,'subdealer' => $subdealer,'user' => $user]);
    }
