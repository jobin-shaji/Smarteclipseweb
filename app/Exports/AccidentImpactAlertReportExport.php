...
class AccidentImpactAlertReportExport implements FromView
{
    ...
    {  
        ...
        if($vehicle!=0)
        {
            $vehicle_details =Vehicle::select('gps_id')->withTrashed()->find($vehicle);
            $single_vehicle_ids = $vehicle_details->gps_id;
        }
        else
        {
            $vehicle_details =Vehicle::select('gps_id','client_id')->withTrashed()->where('client_id',$client)->get();             
	...
	{
       return view('Exports::accident-impact-alert-report', [
            'accidentImpactAlertReportExport' => $this->accidentImpactAlertReportExport
        ]);
	}
